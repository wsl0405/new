package com.zhuo.threads;

import com.zhuo.model.PicObject;
import com.zhuo.utils.DBUtil;
import com.zhuo.utils.MyConverter;
import com.zhuo.utils.Utils;

import static com.zhuo.utils.Constants.*;
...
public class ServerAgentThread extends Thread
{
    ...
    public ServerAgentThread(Socket sc)
    {
        ...
        {
            this.sc=sc;
            in=new DataInputStream(sc.getInputStream()); //输入流，用于接收数据
            out=new DataOutputStream(sc.getOutputStream()); //输出流，用于发送数据
        }
        catch(Exception e)
        {
            e.printStackTrace();
    ...
    {
        try {
            String msg= Utils.readStr(in);//读取客户端的请求信息
            ...
            if(msg.startsWith(UPDATE_NEW))
            {
                ...
                if(res!=null)
                {
                    ...
                    if(bsid==1)
                    {
                        byte[] picTitel = Utils.readBytes(in);
                        String name0=xwid+"_titel.jpg";
                        com.zhuo.utils.FileUtiles.DeleteFolder(picPath+name0);
                        Utils.saveImage(Utils.bytesToBufImg(picTitel),picPath+name0);
                    ...
                    }else if(bsid==2)
                    {
                        ...
                        byte[] pic1 = Utils.readBytes(in);
                        String name1=xwid+"_pic_1.jpg";
                        com.zhuo.utils.FileUtiles.DeleteFolder(picPath+name1);
                        Utils.saveImage(Utils.bytesToBufImg(pic1),picPath+name1);
                    ...
                    }else if(bsid==3)
                    {
                        ...
                        byte[] pic2 = Utils.readBytes(in);
                        String name2=xwid+"_pic_2.jpg";
                        com.zhuo.utils.FileUtiles.DeleteFolder(picPath+name2);
                        Utils.saveImage(Utils.bytesToBufImg(pic2),picPath+name2);
